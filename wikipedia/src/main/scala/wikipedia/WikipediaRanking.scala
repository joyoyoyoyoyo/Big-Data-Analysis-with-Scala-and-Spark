...
object WikipediaRanking {
  ...
  def occurrencesOfLang(lang: String, rdd: RDD[WikipediaArticle]): Int = rdd.aggregate(0)(
    (frequencyCount, wiki) => {
      frequencyCount + (if (wiki.mentionsLanguage(lang)) 1 else 0)
    }, _ + _
  )
  ...
  def rankLangs(langs: List[String], rdd: RDD[WikipediaArticle]): List[(String, Int)] = langs map {
    lang => (lang, WikipediaRanking.occurrencesOfLang(lang, rdd))
  } sortBy(- _._2)
  ...
  def rankLangsReduceByKey(langs: List[String], rdd: RDD[WikipediaArticle]): List[(String, Int)] = {
    ...
    val languageRank = articleMentions.reduceByKey(_ + _)
    languageRank.collect().sortBy(-_._2).toList
  }
